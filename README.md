...
Констурктор:
`constructor(products: IProduct[] = [], selectedProduct: IProduct | null = null)` - констурктор создает новый объект класса ProductCatalog. Первый параметр хранит массив всех товаров каталога (по умолчанию пустой массив). Второй параметр хранит товар, выбранный для подробного отображения (по умолчанию null).
...
Конструктор: 
`constructor(items: IProduct[] = [])` - конструктор "инициализирует" корзину, чтобы она знала, какие товары уже есть внутри. 
...
Конструктор: 
`constructor(payment?: 'card' | 'cash', email?: string, phone?: string, address?: string)` - инициализирует объект покупателя с указанными значениями полей (по умолчанию пустые/неопределенные).
